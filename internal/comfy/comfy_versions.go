...
func comfyVersionSnapshot() map[string]any {
	...
	// 模型清单
	models := map[string][]any{}
	root := filepath.Join(paths.ComfySharedDir, "models")
	...
	// 插件 git 状态(本地,无网络)
	var plugins []any
	cnDir := filepath.Join(paths.ComfyRootDir, "custom_nodes")
	...
	for _, dir := range []string{
		filepath.Join(paths.ComfyRootDir, "user", "default", "workflows"),
		filepath.Join(paths.ComfyRootDir, "workflow_templates"),
	} {
...
// VersionSnapshot 版本/模型/插件快照(导出面,manju 项目体检用)
func VersionSnapshot() map[string]any { return comfyVersionSnapshot() }

// LatestVerCached ComfyUI 最新版本(10 分钟缓存)
func LatestVerCached() string { return comfyLatestVerCached() }
