...
//  2. .venv/pyvenv.cfg:home 解析失败时重写为 ComfyRootDir 旁的 standalone-env
...
func comfySelfHeal() []string {
	...
	healPyvenvCfg()
	var env []string
	if d := filepath.Join(ComfySharedDir, "models", "conditioning"); dirExists(d) {
...
func healExtraModelPaths() {
	...
	path := filepath.Join(ComfyRootDir, "extra_model_paths.yaml")
	want := filepath.ToSlash(ComfySharedDir) + "/"
...
// 无 python.exe 时重写为 ComfyRootDir 旁的 standalone-env(自包含布局)。
func healPyvenvCfg() {
	venvDir := filepath.Join(ComfyRootDir, ".venv")
	...
	want := filepath.Join(filepath.Dir(ComfyRootDir), "standalone-env")
