...
func rebuildVoiceLibIndex() error {
	...
	if err := os.MkdirAll(VoiceLibDir, 0o755); err != nil {
		return err
	}
	var list []map[string]any
	_ = filepath.WalkDir(VoiceLibDir, func(p string, d os.DirEntry, err error) error {
		if err != nil || d.IsDir() {
			return nil
		}
		...
		rel, _ := filepath.Rel(VoiceLibDir, p)
	...
	b, _ := json.MarshalIndent(idx, "", "  ")
	return os.WriteFile(filepath.Join(VoiceLibDir, "index.json"), b, 0o644)
}
