...
func TestOutputDeleteCharScope(t *testing.T) {
	...
	charDir := filepath.Join(ManjuRootDir, proj, "assets", "characters")
	...
	_ = os.WriteFile(filepath.Join(ManjuRootDir, proj, "assets", "asset_map.json"), []byte(amap), 0644)
	...
	b, _ := os.ReadFile(filepath.Join(ManjuRootDir, proj, "assets", "asset_map.json"))
...
func TestOutputDeleteSceneScope(t *testing.T) {
	...
	sceneDir := filepath.Join(ManjuRootDir, proj, "assets", "scenes")
...
func TestCharLibDeleteBatchAndSource(t *testing.T) {
	// CharLibDir 全局临时替换(先例:全局路径局部赋值+defer 恢复,不污染真实库)
	oldLib := CharLibDir
	CharLibDir = filepath.Join(t.TempDir(), "char_lib")
	defer func() { CharLibDir = oldLib }()
	...
	writeAssetFile(t, filepath.Join(ManjuRootDir, proj, "assets", "characters"), "李四.png")
