...
func manjuCacheClear(w http.ResponseWriter, r *http.Request) {
	...
	for _, state := range []string{"run_state.json", "agent_state.json"} {
		sp := filepath.Join(paths.ManjuRootDir, ctx.project, state)
	...
	// 模型权重(../models)绝不触碰。目录来自 paths.ComfySharedDir(与 comfyParams 同源)。
	if advanced {
		...
		for _, sub := range []string{"input", "output"} {
			dir := filepath.Join(paths.ComfySharedDir, sub)
		...
		// 场景图(2026-09-01):场景图按「文件存在」生成无代际——场景卡改卡后旧图不重
		// 生成(趋同旧图残留),清掉后按新卡重生成(场景渲染趋同根治配套)
		sceneDir := filepath.Join(ctx.assetsDir, "scenes")
		if files, bytes, fl := removeTreeEx(sceneDir, &failed); files > 0 || fl > 0 {
			totalFails += fl
			clean = append(clean, map[string]any{"target": "scenes", "files": files, "bytes": bytes, "dir": sceneDir})
		}
		...
		n, b := int64(0), int64(0)
		if entries, err := os.ReadDir(paths.ManjuRootDir); err == nil {
			for _, e := range entries {
				if !e.IsDir() {
					continue
				}
				p := filepath.Join(paths.ManjuRootDir, e.Name(), "run.log")
		...
		if n > 0 {
			clean = append(clean, map[string]any{"target": "runlog", "files": n, "bytes": b, "dir": filepath.Join(paths.ManjuRootDir, "<项目>/run.log")})
		}
