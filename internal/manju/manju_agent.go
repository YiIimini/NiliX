...
var globalSettings atomic.Pointer[config.Settings]

// SetSettings 注入全局设置快照源(2026-09-03 拆包:novel 域 handler 不再挂 Server)
func SetSettings(c *config.Settings) {
	if c != nil {
		globalSettings.Store(c)
	}
}

func settingsSnapshot() config.Settings {
	if c := globalSettings.Load(); c != nil {
		return *c
	}
	if manjuSettingsStore != nil {
		if c, err := manjuSettingsStore.Load(); err == nil {
			return *c
		}
	}
...
func manjuAgentStatePath(project string) string {
	return filepath.Join(paths.ManjuRootDir, project, "agent_state.json")
}
...
// SettingsStore 全局设置存储访问器(island 等 api 域消费)
func SettingsStore() *config.Store { return manjuSettingsStore }

// SetSettingsStore 测试注入用(island 域测试)
func SetSettingsStore(s *config.Store) { manjuSettingsStore = s }
