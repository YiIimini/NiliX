...
func manjuDiagnoseDir() string {
	return filepath.Join(paths.ManjuRootDir, "logs", "diagnose")
}
...
func manjuWriteDiagnoseSnapshot(project, episode string) {
	...
	cfgPath := filepath.Join(paths.ManjuRootDir, project, "config.json")
...
func manjuWriteCrash(where string, msg string, stack []byte) {
	dir := filepath.Join(paths.ManjuRootDir, "logs")
