...
func TestNewManjuCtxComfyOutputPriority(t *testing.T) {
	oldURL, oldIn, oldOut := comfy.ComfyURL(), comfy.In(), comfy.Out()
	...
	comfy.SetComfyParams("", filepath.Join(dir, "live-in"), filepath.Join(dir, "live-out"))
...
// 回归:新项目默认 comfy_input/comfy_output 跟随 paths.ComfySharedDir(而非硬编码 Desktop 路径)
