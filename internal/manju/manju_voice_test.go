...
// 2026-09-03 配音去 AI 味:①音色库每个 key 的参考文本与表演化短语齐备(文本兜底中性句
// 允许,但主档位必须有情感文本;短语表全覆盖且非静态播报腔);②narrator 短语保留讲述
// 节奏;③方言组短语带口音描述。
func TestVoiceGenTextAndPhrasesUpgrade(t *testing.T) {
	for _, v := range manjuVoiceLib {
		if manjuVoiceGenTextFor(v.Key) == "" {
			t.Errorf("音色 %s 参考文本为空", v.Key)
		}
		ph := manjuVoicePhraseFor(v.Key)
		if ph == "" {
			t.Errorf("音色 %s 短语缺失(表演化表全覆盖)", v.Key)
		}
	}
	// 主档位必须用情感文本(非兜底中性句)——中性句兜底只允许变体/新增未登记 key
	for _, k := range []string{"male_sun", "female_warm", "male_mag", "male_elder",
		"male_deep", "beast_cute", "child_girl", "male_narrator"} {
		if manjuVoiceGenTextFor(k) == manjuVoiceGenText {
			t.Errorf("主档位 %s 应用情感化参考文本,而非中性兜底句", k)
		}
	}
	// 短语去平板化:主档位短语应含念白风格描述(talks like/speaks with/cadence 类)
	for _, k := range []string{"male_sun", "female_warm", "male_deep"} {
		ph := manjuVoicePhraseFor(k)
		if !strings.Contains(ph, "talks") && !strings.Contains(ph, "speaks") && !strings.Contains(ph, "speaking") && !strings.Contains(ph, "each word") {
			t.Errorf("音色 %s 表演化短语应含念白风格描述, got %q", k, ph)
		}
	}
	// narrator 讲述节奏(非死板 neutral)
	if ph := manjuVoicePhraseFor("male_narrator"); !strings.Contains(ph, "engaging") {
		t.Errorf("narrator 短语应含讲述节奏描述, got %q", ph)
	}
}
