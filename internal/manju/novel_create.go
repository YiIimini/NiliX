...
// novelRoot 小说库根目录(审计 H13):一律读可配置的 paths.NovelRootDir(main 按 settings/自包含
// 解析注入),不再硬编码 C:\Mi\Ai\WorkBench\Novel——此前设置改 novel_root 后网页小说仍写
// 旧目录,fs 白名单/书架/转剧本全线脱节
func novelRoot() string {
	if strings.TrimSpace(paths.NovelRootDir) != "" {
		return paths.NovelRootDir
	}
...
func handleNovelCreate(w http.ResponseWriter, r *http.Request) {
	var req struct {
...
chapters 必须恰好 %d 条,no 从 1 连续递增;卷数=%d;首卷埋线、末卷收线,每卷末章是卷末高潮,章章有钩子。`,
	...
	// 技能规范副本:创作指令卡 / 章节写作规范(模板存在则复制,缺失静默);路径跟随 paths.NovelSkillDir(配置/自包含/legacy 三链解析)
	skillRef := filepath.Join(paths.NovelSkillDir, "references")
...
func handleNovelChapter(w http.ResponseWriter, r *http.Request) {
	var req struct {
		Title string `json:"title"`
		No    int    `json:"no"`
	}
...
func handleNovelProgress(w http.ResponseWriter, r *http.Request) {
...
func handleNovelAnalyze(w http.ResponseWriter, r *http.Request) {
	var req struct {
		Genre string `json:"genre"`
		Style string `json:"style"`
	}
...
func handleNovelReview(w http.ResponseWriter, r *http.Request) {
	var req struct {
		Title string `json:"title"`
		No    int    `json:"no"`
	}
...
func handleNovelAuto(w http.ResponseWriter, r *http.Request) {
	var req struct {
		Title string `json:"title"`
	}
	_ = json.NewDecoder(r.Body).Decode(&req)
...
func handleNovelAutoStop(w http.ResponseWriter, r *http.Request) {
	var req struct {
		Title string `json:"title"`
	}
	_ = json.NewDecoder(r.Body).Decode(&req)
...
func handleNovelAutoStatus(w http.ResponseWriter, r *http.Request) {
	title := strings.TrimSpace(r.URL.Query().Get("title"))
	if title == "" {
		writeErr(w, http.StatusBadRequest, "missing title")
		return
...
func HandleNovelStatusAll(w http.ResponseWriter, r *http.Request) {
	out := map[string]any{}
...
func handleNovelDelete(w http.ResponseWriter, r *http.Request) {
	var body struct {
		Title string `json:"title"`
	}
