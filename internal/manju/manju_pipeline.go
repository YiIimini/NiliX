...
func (ctx *manjuCtx) freeComfyModels(lg *manjuLogger) {
	...
	resp, err := ctx.comfy.HTTP.Do(req)
	if err != nil {
		if lg != nil {
...
// 2026-09-03 表演化重写(配音去 AI 味):旧短语是纯静态音色描述(clear and steady/
// calm and deep),H3 按描述念白 → 全员平板播报腔。重写为「音色+这个人怎么说话」
// 的性格化念白风格(talks like/speaks with),驱动 H3 带人味地念;旁白保留沉稳但
// 加讲述节奏(engaging cadence)防死板。内心独白的剧情情绪仍由 delivery 映射叠加。
func manjuVoicePhraseFor(key string) string {
	phrases := map[string]string{
		"beast_cute":      "a cute chirpy creature voice, talks in quick bright excited bursts",
		"beast_cute_2":    "a soft round baby-ish creature voice, coos and giggles as it talks",
		"child_boy":       "a little boy's voice, high and lively, chatters with fearless energy",
		"child_boy_2":     "a little boy's voice, piping quick and bold, blurts things out",
		"child_girl":      "a little girl's voice, sweet and bouncy, sings through her words",
		"child_girl_2":    "a little girl's voice, soft and clinging, talks in a honeyed drawl",
		"boy_teen":        "a teenage boy's voice, bright and brash, talks fast with eager swagger",
		"boy_teen_2":      "a teenage boy's voice, reedy and impatient, words tumbling out quickly",
		"girl_lively":     "a young girl's voice, lively and sparkling, chats like a happy friend",
		"girl_lively_2":   "a young girl's voice, bubbly and teasing, talks with playful bounce",
		"male_sun":        "a young man's voice, warm and easygoing, talks like a cheerful reliable friend",
		"male_sun_2":      "a young man's voice, slightly husky, speaks in a relaxed unhurried drawl",
		"male_sun_3":      "a young man's voice, resonant and spirited, speaks with easy confidence",
		"female_warm":     "a young woman's voice, soft and caring, speaks gently as if comforting someone",
		"female_warm_2":   "a young woman's voice, clear and thoughtful, talks in a composed friendly manner",
		"male_mag":        "a middle-aged man's voice, deep and unhurried, speaks with measured authority",
		"male_mag_2":      "a middle-aged man's voice, low and rumbling, talks slowly weighing each word",
		"male_mag_3":      "a middle-aged man's voice, gravelly and forceful, barks out words with rough vigor",
		"female_mature":   "a mature woman's voice, smooth and shrewd, speaks with brisk knowing poise",
		"female_mature_2": "a mature woman's voice, warm and capable, talks in a brisk motherly way",
		"male_elder":      "an old man's voice, weathered and kind, talks slowly with the rasp of age and a chuckle underneath",
		"male_elder_2":    "an old man's voice, deep and heavy, speaks haltingly as memories surface",
		"female_elder":    "an elderly woman's voice, warm and crackly, chats fondly like a grandmother telling stories",
		"female_elder_2":  "an elderly woman's voice, dry and creaky, speaks in slow clipped nostalgia",
		"male_deep":       "a cold menacing man's voice, low and deliberate, each word pressed down with quiet threat",
		...
		"cn_shaanxi":  "a woman speaking Mandarin with a bright Shaanxi accent, blunt and warm",
		"cn_sichuan":  "a man speaking Mandarin with a lively Sichuan accent, quick with playful slang",
		"cn_henan":    "a man speaking Mandarin with an earthy Henan accent, plain and good-natured",
		"cn_guangxi":  "a man speaking Mandarin with a soft Guangxi accent, easygoing and lilting",
		"cn_hunan":    "a man speaking Mandarin with a spirited Hunan accent, fiery and fast",
		"hk_male":     "a man speaking Cantonese with a Hong Kong accent, breezy street-smart cadence",
		"hk_female":   "a woman speaking Cantonese with a Hong Kong accent, crisp vivacious cadence",
		"tw_male":     "a man speaking Mandarin with a gentle Taiwanese accent, soft lilting drawl",
		"tw_female":   "a woman speaking Mandarin with a gentle Taiwanese accent, sweet lingering cadence",
		"male_narrator":   "a calm storytelling voice with an engaging unhurried cadence, like a seasoned audiobook narrator",
		"female_narrator": "a calm storytelling voice with an engaging unhurried cadence, like a seasoned audiobook narrator",
	}
	return phrases[key]
}
...
// 2026-09-03:Q 版资产已改全员渲染,Q 生成不再用此判定;保留供测试与潜在
// 正角语义查询(内心戏画面路由由技能侧分镜契约管)。
...
func manjuQFormPrompt(m map[string]any) string {
	if manjuIsBeast(m) {
		...
		} else if low2 := strings.ToLower(str(m["species"]) + " " + str(m["image_prompt"])); strings.Contains(low2, "water") ||
			strings.Contains(low2, "rain") || strings.Contains(low2, "雨") || strings.Contains(low2, "水") ||
			strings.Contains(low2, "translucent") {
			// 2026-09-03 水体系物种适配(万物皆可反悔 未落·雨灵 Q 版出入实锤):
			// 水灵/雨灵/水体描述的兽,通用 plush 毛绒身体模板与其形象直接冲突
			// (Q 版渲染成毛绒怪)。换成光滑圆润水体形态,glowing 眼保留(辨识锁)。
			p = "3D rendered cute chibi collectible toy figure, small adorable round chibi water-sprite creature with a smooth glossy rounded water-blue body, oversized head with big sparkling glowing droplet eyes, short stubby legs and tiny round paws, a short curly tail fading into fine drizzle, standing on a clean plain white background"
		}
...
func copyFile(src, dst string) error { return util.CopyFile(src, dst) }
...
func stageAssets(ctx *manjuCtx, lg *manjuLogger) error {
	...
	for _, c := range chars {
		...
		// 2026-09-03 用户规则再升级:Q 版全员渲染——全部角色(含配角/群演)都生成
		// Q 版资产(技能侧 q_form 全员必填;审片/分镜/未来画面路由随时可用);
		// 推翻 2026-09-01「仅正角」规则。内心戏画面路由不变:正角 Q 版/配角写实
		// (画面规则由技能侧分镜契约管,资产层全量备齐)。
...
func (ctx *manjuCtx) shotCondFingerprintAt(s manjuShot, w, h int) string {
	...
	// 2026-09-03 配音去 AI 味·指纹维度补全:音色参考音频(charVoiceNames 挂 ref_audios)
	// 之前不进指纹——音色库念白文本升级重生成后(文件名不变,内容/韵律已变),旧条件
	// 缓存仍命中,编码继续用旧平板韵律音频,升级静默失效。音频 mtime+size 入指纹:
	// 参考音频再生成的镜自动 stale 重编重渲。
	for _, rel := range ctx.charVoiceNames(s) {
		if fi, err := os.Stat(filepath.Join(ctx.comfyInput, filepath.FromSlash(rel))); err == nil {
			refs = append(refs, fmt.Sprintf("voice:%s@%d:%d", rel, fi.ModTime().UnixNano(), fi.Size()))
		}
	}
...
func (ctx *manjuCtx) voiceLibAuthoritative(rel string) string {
	if paths.VoiceLibDir != "" {
		return filepath.Join(paths.VoiceLibDir, filepath.FromSlash(rel))
	}
...
func (ctx *manjuCtx) genVoiceLibAudio(key string) error {
	...
	// 旧版无 meta 文件同样触发重生成。
	if fileExists(out) {
		if b, err := os.ReadFile(metaPath); err == nil && strings.TrimSpace(string(b)) == text {
	...
	args := []string{"voice-gen", "--text", text, "--voice", item.Name, "--out", out}
	if item.Pitch != "" {
	...
	_ = os.WriteFile(metaPath, []byte(text), 0o644) // 念白文本 meta(升级自愈锚)
	ctx.syncVoiceLibToComfy(voiceLibRel(item.Key))
	return nil
}
...
func (ctx *manjuCtx) runVoiceover(lg *manjuLogger) error {
	...
	if ff := filepath.Join(paths.ComfyRootDir, ".venv", "Scripts", "ffmpeg.exe"); fileExists(ff) {
...
func ensureMediaHelper() string {
	dir := filepath.Join(paths.ManjuRootDir, "logs", "media")
...
func manjuCreateProject(name, novel, apiKey string) (string, string, bool) {
	...
	projDir := filepath.Join(paths.ManjuRootDir, clean)
...
func manjuDefaultConfig(name, novelFile, novelDir, apiKey string) map[string]any {
	return map[string]any{
		...
		"paths": map[string]any{
			"workdir":      filepath.Join(paths.ManjuRootDir, name),
			"novel":        novelFile,
			"novel_dir":    novelDir,
			"analysis":     filepath.Join(paths.ManjuRootDir, name, "analysis"),
			"assets":       filepath.Join(paths.ManjuRootDir, name, "assets"),
			"clips":        filepath.Join(paths.ManjuRootDir, name, "clips"),
			...
			"comfy_input":  filepath.Join(paths.ComfySharedDir, "input"),
			"comfy_output": filepath.Join(paths.ComfySharedDir, "output"),
			"outline":      "", "setting": "",
		},
	}
}
