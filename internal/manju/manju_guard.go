...
//   - config 必须解析为 paths.ManjuRootDir/<项目>/config.json（禁止任意路径读/写 JSON）
//   - novel 必须位于 paths.NovelRootDir 之内（禁止任意文件整读，防密钥泄漏与 OOM）
...
func manjuGuardConfig(configPath string) (string, error) {
	...
	root := filepath.Clean(paths.ManjuRootDir)
	if cp == root || !strings.HasPrefix(cp, root+string(filepath.Separator)) {
		return "", fmt.Errorf("config 路径必须在项目根目录 %s 内", paths.ManjuRootDir)
	}
...
func manjuGuardNovel(novelPath string) (string, error) {
	...
	root := filepath.Clean(paths.NovelRootDir)
	if np == root || !strings.HasPrefix(np, root+string(filepath.Separator)) {
		return "", fmt.Errorf("小说路径必须在小说库根目录 %s 内", paths.NovelRootDir)
	}
	return np, nil
}
