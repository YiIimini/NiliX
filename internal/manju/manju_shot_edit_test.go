...
func TestShotEditDialogueSync(t *testing.T) {
	...
	b, _ := os.ReadFile(filepath.Join(paths.ManjuRootDir, "zz_shot_edit_test", "analysis", "EP01_direct_plan.json"))
...
func TestShotEditPromptManualAndGuard(t *testing.T) {
	...
	b, _ := os.ReadFile(filepath.Join(paths.ManjuRootDir, "zz_shot_edit_test2", "analysis", "EP01_direct_plan.json"))
	var plan map[string]any
	_ = json.Unmarshal(b, &plan)
...
func TestScenesListAPI(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(paths.ManjuRootDir, "zz_scenes_test", "assets", "scenes", "大厅.png"), []byte("png"), 0o644)
