...
// newHealthResidueProj 构造临时项目(位于 paths.ManjuRootDir 下:run_state/run.log 均按
// paths.ManjuRootDir/<项目>/ 解析,TempDir 无法覆盖),返回 (项目名, config路径, 清理函数)
...
func TestHealthLogsResidueFix(t *testing.T) {
	...
	crashPath := filepath.Join(paths.ManjuRootDir, "logs", "crash.log")
