...
func SetFSRoots(roots ...string) {
	...
	// 漫剧项目根恒允许(api 包内部)
	if r := fsNormRoot(paths.ManjuRootDir); r != "" && !seen[strings.ToLower(r)] {
		fsRoots = append(fsRoots, r)
	}
}
...
func handleFSAnalyze(w http.ResponseWriter, r *http.Request) {
	dir := r.URL.Query().Get("dir")
	if dir == "" {
		writeErr(w, http.StatusBadRequest, "missing dir")
		return
...
func handleFSSelect(w http.ResponseWriter, r *http.Request) {
	p, err := pickDir()
	if err != nil {
...
func handleFSMedia(w http.ResponseWriter, r *http.Request) {
	dir := r.URL.Query().Get("dir")
	if dir == "" {
		writeErr(w, http.StatusBadRequest, "missing dir")
		return
...
// RegisterFsRoutes 文件服务路由(2026-09-03 拆包:api.Server 接线)
func RegisterFsRoutes(mux *http.ServeMux) {
	mux.HandleFunc("GET /api/fs/list", handleFSList)
	mux.HandleFunc("GET /api/fs/analyze", handleFSAnalyze)
	mux.HandleFunc("POST /api/fs/select", handleFSSelect)
	mux.HandleFunc("GET /api/fs/read", handleFSRead)
	mux.HandleFunc("GET /api/fs/file", handleFSFile)
	mux.HandleFunc("GET /api/fs/media", handleFSMedia)
}
