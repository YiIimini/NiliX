...
func TestCharLibIndexLifecycle(t *testing.T) {
	dir := t.TempDir()
	old := paths.ManjuRootDir
	paths.ManjuRootDir = dir
	paths.CharLibDir = filepath.Join(dir, "asset_lib", "characters")
	defer func() { paths.ManjuRootDir = old; paths.CharLibDir = "" }()

	// 造两个角色目录
	for _, name := range []string{"季一星", "阿影"} {
		d := filepath.Join(paths.CharLibDir, name)
	...
	// 索引内容:两角色,七字段+资产清单
	b, err := os.ReadFile(filepath.Join(paths.CharLibDir, "index.json"))
	if err != nil {
	...
	_ = os.Remove(filepath.Join(paths.CharLibDir, "index.json"))
...
func TestVoiceLibIndexRebuild(t *testing.T) {
	dir := t.TempDir()
	old := paths.VoiceLibDir
	paths.VoiceLibDir = filepath.Join(dir, "voices")
	defer func() { paths.VoiceLibDir = old }()
	_ = os.MkdirAll(filepath.Join(paths.VoiceLibDir, "audio"), 0o755)
	_ = os.WriteFile(filepath.Join(paths.VoiceLibDir, "audio", "male_sun.mp3"), []byte("x"), 0o644)
	if err := rebuildVoiceLibIndex(); err != nil {
		t.Fatal(err)
	}
	b, err := os.ReadFile(filepath.Join(paths.VoiceLibDir, "index.json"))
	if err != nil {
