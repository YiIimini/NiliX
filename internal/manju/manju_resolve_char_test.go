...
func TestInjectAudioTimbrePhrasesFix(t *testing.T) {
	...
	if !strings.Contains(out, "a young woman's voice, soft and caring") {
		t.Fatalf("金珠错配短语未校正: %s", out[:200])
	}
	if !strings.Contains(out, "a young man's voice, slightly husky") {
		t.Fatalf("石敢当变体短语未生效: %s", out[:250])
	}
	if !strings.Contains(out, "an old man's voice, weathered and kind") {
