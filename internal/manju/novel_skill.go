...
// paths.NovelSkillDir 技能目录(违规词库/负面提示词库/章节写作规范/qa 脚本);见 paths.go

// novelHardBanned 违规词库硬禁词(## [硬禁] 分类下词条;增删词库即生效)
func novelHardBanned() []string {
	b, err := os.ReadFile(filepath.Join(paths.NovelSkillDir, "wordbank", "违规词库.md"))
	if err != nil {
		return nil
	}
	var words []string
...
func novelWritingSpec() string {
	b, err := os.ReadFile(filepath.Join(paths.NovelSkillDir, "references", "章节写作规范.md"))
	if err != nil {
		return ""
	}
	return string(b)
...
func runNovelQACheck(proj string) string {
	script := filepath.Join(paths.NovelSkillDir, "scripts", "qa_check.py")
